...
p0(无证据, per-gold)=[-9.15]
pF(全轨迹, per-gold)=[-2.17]
通路独立 l [Franz Liszt] per-gold=[-13.41]
通路独立 l [Life of Franz Liszt] per-gold=[-3.21]
通路独立 l [Freemasonry] per-gold=[-14.71]
...
◆ 旧标记: cls=effective d=-1.223e-05 l=-0.0009432 f=-1.233e-05 N=1.0 g=- lineage=✓ necessary=yes
...
◆ 旧标记: cls=effective d=0.0006215 l=0.0002342 f=0.001091 N=0.935 g=★ lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
◆ 旧标记: cls=redundant_irr d=-0.0002467 l=-0.0001592 f=-1.284e-05 N=1.0 g=- lineage=- necessary=no
◆ RSCC[v2.1] per-gold 明细:
    Priest                         logP(ref)=-14.71 → logP(cf)=-9.055  Δ=-5.656  反升286.1x ‼
  max drop=-%  coverage=0 D_pos=0  label=harmful → 移除
◆ LOO:  d(per-gold)=[+0.11]  D_max=0.1138 cov=1
  [结构: necessary=no pathway=franz liszt]

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Franz Liszt〕起始 l_alone(per-gold)=[-13.41]
  块#0: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Life of Franz Liszt〕起始 l_alone(per-gold)=[-3.21]
  块#1: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Freemasonry〕起始 l_alone(per-gold)=[-14.71]
  块#2 结构必要=否: d=[-5.66] max-drop=-% cov=0 [harmful] ⇒ harmful → 移除
  终态[Freemasonry] l(per-gold)=[-9.15]
  双通道小结: 结构必要块=['0', '1'] | 概率移除块=['2'] | 两通道无冲突
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-8.57]
pF(全轨迹, per-gold)=[-1.08]
通路独立 l [Siege of Vicksburg] per-gold=[-4.61]
通路独立 l [Carter L. Stevenson] per-gold=[-1.51]
...
◆ 旧标记: cls=effective d=0.4209 l=-0.06237 f=0.4209 N=1.0 g=★ lineage=✓ necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
◆ 旧标记: cls=effective d=0.2103 l=0.2154 f=0.7008 N=0.75 g=- lineage=- necessary=no
◆ RSCC[v2.1] per-gold 明细:
    John C. Pemberton              logP(ref)=-1.511 → logP(cf)=-8.566  Δ=+7.055  drop=+99.9% ★
  max drop=99.91%  coverage=1 D_pos=7.055  label=informative → 保留
◆ LOO:  d(per-gold)=[+3.49]  D_max=3.485 cov=1
  [结构: necessary=no pathway=siege of vicksburg]

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Siege of Vicksburg〕起始 l_alone(per-gold)=[-4.61]
  块#0: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Carter L. Stevenson〕起始 l_alone(per-gold)=[-1.51]
  块#1 结构必要=否: d=[+7.06] max-drop=99.91% cov=1 [informative] ⇒ Informative → 保留
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-11.91]
pF(全轨迹, per-gold)=[-17.55]
通路独立 l [Ron Howard] per-gold=[-15.26]
通路独立 l [?film] per-gold=[-16.83]
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=-0.0004679 l=0.0002958 f=-0.0004034 N=1.0 g=- lineage=✓ necessary=n/a
◆ RSCC[v2.1] per-gold 明细:
    The Journey                    logP(ref)=-15.26 → logP(cf)=-11.96  Δ=-3.296  反升27.0x ‼
  max drop=-%  coverage=0 D_pos=0  label=harmful → 移除
◆ LOO:  d(per-gold)=[-0.56]  D_max=-0.5555 cov=0
...
◆ 旧标记: cls=redundant_dup d=-0.0002236 l=-0.0001929 f=-0.0009565 N=0.0 g=- lineage=- necessary=n/a
◆ RSCC[v2.1] per-gold 明细:
    The Journey                    logP(ref)=-16.83 → logP(cf)=-12.13  Δ=-4.696  反升109.5x ‼
  max drop=-%  coverage=0 D_pos=0  label=harmful → 移除
◆ LOO:  d(per-gold)=[-2.11]  D_max=-2.111 cov=0
  [结构: necessary=n/a pathway=ron howard]

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Ron Howard〕起始 l_alone(per-gold)=[-15.26]
  块#0 结构=n/a(通路未达): d=[-3.30] max-drop=-% cov=0 [harmful] ⇒ harmful → 移除
〔通路 ?film〕起始 l_alone(per-gold)=[-16.83]
  块#1 结构=n/a(通路未达): d=[-4.70] max-drop=-% cov=0 [harmful] ⇒ harmful → 移除
  终态[Ron Howard] l(per-gold)=[-12.02]
  终态[?film] l(per-gold)=[-11.98]
  双通道小结: 结构必要块=无 | 概率移除块=['0', '1'] | 两通道无冲突
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-16.31]
pF(全轨迹, per-gold)=[-1.86]
通路独立 l [Ron Howard] per-gold=[-20.63]
通路独立 l [Child prodigy] per-gold=[-1.37]
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=harmful d=-0.008602 l=-0.25 f=-0.008602 N=1.0 g=- lineage=- necessary=n/a
◆ RSCC[v2.1] per-gold 明细:
    Village of the Giants          logP(ref)=-20.63 → logP(cf)=-16.31  Δ=-4.321  反升75.3x ‼
  max drop=-%  coverage=0 D_pos=0  label=harmful → 移除
◆ LOO:  d(per-gold)=[-0.65]  D_max=-0.6459 cov=0
  [结构: necessary=n/a pathway=ron howard]
...
◆ 旧标记: cls=effective d=0.5517 l=0.5517 f=0.7931 N=1.0 g=★ lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Ron Howard〕起始 l_alone(per-gold)=[-20.63]
  块#0 结构=n/a(通路未达): d=[-4.32] max-drop=-% cov=0 [harmful] ⇒ harmful → 移除
〔通路 Child prodigy〕起始 l_alone(per-gold)=[-1.37]
  块#1: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
  终态[Ron Howard] l(per-gold)=[-16.34]
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-9.39]
pF(全轨迹, per-gold)=[-2.38]
通路独立 l [Tupac Shakur] per-gold=[-2.77]
通路独立 l [Kirk M. Petruccelli] per-gold=[-4.15]
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.6007 l=0.3038 f=0.6007 N=1.0 g=★ lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
◆ 旧标记: cls=effective d=0.0803 l=0.0803 f=0.3735 N=1.0 g=- lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Tupac Shakur〕起始 l_alone(per-gold)=[-2.77]
  块#0: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Kirk M. Petruccelli〕起始 l_alone(per-gold)=[-4.15]
  块#1: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-5.81]
pF(全轨迹, per-gold)=[-1.33]
通路独立 l [Ethiopian birr] per-gold=[-14.67]
通路独立 l [Ethiopia] per-gold=[-2.21]
...
◆ 旧标记: cls=effective d=0.215 l=0.009404 f=0.2154 N=1.0 g=- lineage=✓ necessary=n/a
◆ RSCC[v2.1] per-gold 明细:
    Hailemariam Desalegn           logP(ref)=-14.67 → logP(cf)=-5.807  Δ=-8.859  反升7039.9x ‼
  max drop=-%  coverage=0 D_pos=0  label=harmful → 移除
◆ LOO:  d(per-gold)=[+0.77]  D_max=0.7683 cov=1
...
◆ 旧标记: cls=effective d=0.1347 l=0.1364 f=0.3424 N=0.944 g=★ lineage=- necessary=n/a
◆ RSCC[v2.1] per-gold 明细:
    Hailemariam Desalegn           logP(ref)=-2.214 → logP(cf)=-5.803  Δ=+3.589  drop=+97.2% ★
  max drop=97.24%  coverage=1 D_pos=3.589  label=informative → 保留
◆ LOO:  d(per-gold)=[+13.21]  D_max=13.21 cov=1
  [结构: necessary=n/a pathway=ethiopian birr]

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Ethiopian birr〕起始 l_alone(per-gold)=[-14.67]
  块#0 结构=n/a(通路未达): d=[-8.86] max-drop=-% cov=0 [harmful] ⇒ harmful → 移除
〔通路 Ethiopia〕起始 l_alone(per-gold)=[-2.21]
  块#1 结构=n/a(通路未达): d=[+3.59] max-drop=97.24% cov=1 [informative] ⇒ Informative → 保留
  终态[Ethiopian birr] l(per-gold)=[-5.81]
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-18.65 -18.88 -18.77 -19.80 -20.16 -19.86 -20.01 -19.94 -19.46 -19.22 -18.27 -18.27 -17.87 -18.06 -17.07 -17.15 -13.27]
pF(全轨迹, per-gold)=[-5.43 -5.44 -5.08 -6.38 -9.84 -5.80 -9.47 -10.45 -10.05 -5.09 -8.27 -10.76 -8.76 -7.37 -6.79 -6.28 -4.54]
通路独立 l [Brad Stevens] per-gold=[-22.66 -22.65 -22.75 -23.72 -23.41 -22.68 -23.20 -23.19 -22.29 -22.44 -22.08 -23.16 -22.35 -22.82 -21.52 -21.87 -17.18]
通路独立 l [Butler Bulldogs men's ba] per-gold=[-2.72 -7.27 -7.05 -6.57 -8.38 -8.57 -7.81 -8.38 -6.93 -7.37 -5.65 -7.21 -7.32 -6.14 -4.40 -4.12 -2.45]
通路独立 l [1957 NBA Finals] per-gold=[-5.93 -4.00 -4.14 -6.11 -9.33 -4.90 -9.34 -10.74 -10.98 -3.26 -8.38 -10.51 -8.42 -7.15 -6.54 -6.78 -4.79]
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.05418 l=0.02154 f=0.05387 N=1.0 g=- lineage=✓ necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
◆ 旧标记: cls=effective d=0.1863 l=-0.01321 f=0.219 N=0.935 g=★ lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
◆ 旧标记: cls=effective d=0.08858 l=0.0945 f=0.3157 N=0.516 g=- lineage=- necessary=no
◆ RSCC[v2.1] per-gold 明细:
    1957 NBA Finals                logP(ref)=-5.927 → logP(cf)=-18.58  Δ=+12.657  drop=+100.0% ★
    1959 NBA Finals                logP(ref)=-4 → logP(cf)=-18.76  Δ=+14.758  drop=+100.0% ★
    1960 NBA Finals                logP(ref)=-4.14 → logP(cf)=-18.67  Δ=+14.530  drop=+100.0% ★
    1961 NBA Finals                logP(ref)=-6.106 → logP(cf)=-19.52  Δ=+13.419  drop=+100.0% ★
    1962 NBA Finals                logP(ref)=-9.333 → logP(cf)=-19.88  Δ=+10.547  drop=+100.0% ★
    1963 NBA Finals                logP(ref)=-4.902 → logP(cf)=-19.7  Δ=+14.801  drop=+100.0% ★
    1964 NBA Finals                logP(ref)=-9.336 → logP(cf)=-19.79  Δ=+10.459  drop=+100.0% ★
    1965 NBA Finals                logP(ref)=-10.74 → logP(cf)=-19.95  Δ=+9.211  drop=+100.0% ★
    1966 NBA Finals                logP(ref)=-10.98 → logP(cf)=-19.46  Δ=+8.475  drop=+100.0% ★
    1968 NBA Finals                logP(ref)=-3.258 → logP(cf)=-19.22  Δ=+15.958  drop=+100.0% ★
    1969 NBA Finals                logP(ref)=-8.384 → logP(cf)=-18.37  Δ=+9.982  drop=+100.0% ★
    1974 NBA Finals                logP(ref)=-10.51 → logP(cf)=-18.57  Δ=+8.062  drop=+100.0% ★
    1976 NBA Finals                logP(ref)=-8.417 → logP(cf)=-17.82  Δ=+9.406  drop=+100.0% ★
    1981 NBA Finals                logP(ref)=-7.145 → logP(cf)=-18.11  Δ=+10.967  drop=+100.0% ★
    1984 NBA Finals                logP(ref)=-6.541 → logP(cf)=-17.39  Δ=+10.848  drop=+100.0% ★
    1986 NBA Finals                logP(ref)=-6.781 → logP(cf)=-17.01  Δ=+10.228  drop=+100.0% ★
    2008 NBA Finals                logP(ref)=-4.791 → logP(cf)=-13.06  Δ=+8.271  drop=+100.0% ★
  max drop=100%  coverage=1 D_pos=11.33  label=informative → 保留
◆ LOO:  d(per-gold)=[-1.53 +2.60 +3.43 +0.64 -1.16 +3.77 -1.49 -1.98 -2.63 +2.29 -2.40 -0.98 +0.48 -0.63 -0.96 -0.92 -2.64]  D_max=3.769 cov=0.3529
  [结构: necessary=no pathway=brad stevens]

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Brad Stevens〕起始 l_alone(per-gold)=[-22.66 -22.65 -22.75 -23.72 -23.41 -22.68 -23.20 -23.19 -22.29 -22.44 -22.08 -23.16 -22.35 -22.82 -21.52 -21.87 -17.18]
  块#0: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Butler Bulldogs men's ba〕起始 l_alone(per-gold)=[-2.72 -7.27 -7.05 -6.57 -8.38 -8.57 -7.81 -8.38 -6.93 -7.37 -5.65 -7.21 -7.32 -6.14 -4.40 -4.12 -2.45]
  块#1: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 1957 NBA Finals〕起始 l_alone(per-gold)=[-5.93 -4.00 -4.14 -6.11 -9.33 -4.90 -9.34 -10.74 -10.98 -3.26 -8.38 -10.51 -8.42 -7.15 -6.54 -6.78 -4.79]
  块#2 结构必要=否: d=[+12.66 +14.76 +14.53 +13.42 +10.55 +14.80 +10.46 +9.21 +8.47 +15.96 +9.98 +8.06 +9.41 +10.97 +10.85 +10.23 +8.27] max-drop=100% cov=1 [informative] ⇒ Informative → 保留
  双通道小结: 结构必要块=['0', '1'] | 概率移除块=无 | 两通道无冲突
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-8.48]
pF(全轨迹, per-gold)=[-2.00]
通路独立 l [Feu de joie] per-gold=[-1.58]
通路独立 l [Jackson Pollock] per-gold=[-4.43]
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.2558 l=0.3335 f=0.2524 N=1.0 g=★ lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.07564 l=0.07564 f=-0.001914 N=1.0 g=- lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Feu de joie〕起始 l_alone(per-gold)=[-1.58]
  块#0: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Jackson Pollock〕起始 l_alone(per-gold)=[-4.43]
  块#1: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
  双通道小结: 结构必要块=['0', '1'] | 概率移除块=无 | 两通道无冲突
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-9.97 -10.42]
pF(全轨迹, per-gold)=[-2.50 -12.57]
通路独立 l [Randy Jackson] per-gold=[-3.81 -12.98]
通路独立 l [Eclipse Tour] per-gold=[-10.42 -13.25]
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.1999 l=0.09124 f=0.1999 N=1.0 g=★ lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
◆ 旧标记: cls=effective d=-0.02772 l=-0.02158 f=-0.0005155 N=1.0 g=- lineage=✓ necessary=n/a
◆ RSCC[v2.1] per-gold 明细:
    Bass guitar                    logP(ref)=-10.42 → logP(cf)=-5.897  Δ=-4.522  反升92.0x ‼
    Vocals                         logP(ref)=-13.25 → logP(cf)=-10.28  Δ=-2.964  反升19.4x ‼
  max drop=-%  coverage=0 D_pos=0  label=harmful → 移除
◆ LOO:  d(per-gold)=[-0.66 -2.33]  D_max=-0.6649 cov=0
...
```
(送评归属文本: 4 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.01249 l=0.006449 f=0.1094 N=0.9 g=- lineage=- necessary=n/a
◆ RSCC[v2.1] per-gold 明细:
    Bass guitar                    logP(ref)=-10.42 → logP(cf)=-18.83  Δ=+8.409  drop=+100.0% ★
    Vocals                         logP(ref)=-13.25 → logP(cf)=-16.28  Δ=+3.031  drop=+95.2% ★
  max drop=99.98%  coverage=1 D_pos=5.72  label=informative → 保留
◆ LOO:  d(per-gold)=[+2.04 +0.97]  D_max=2.038 cov=1
  [结构: necessary=n/a pathway=eclipse tour]

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Randy Jackson〕起始 l_alone(per-gold)=[-3.81 -12.98]
  块#0: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Eclipse Tour〕起始 l_alone(per-gold)=[-10.42 -13.25]
  块#2 结构=n/a(通路未达): d=[+8.41 +3.03] max-drop=99.98% cov=1 [informative] ⇒ Informative → 保留
  块#1 结构=n/a(通路未达): d=[-4.52 -2.96] max-drop=-% cov=0 [harmful] ⇒ harmful → 移除
  终态[Eclipse Tour] l(per-gold)=[-6.99 -10.36]
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-9.39]
pF(全轨迹, per-gold)=[-2.62]
通路独立 l [Tupac Shakur] per-gold=[-3.38]
通路独立 l [Kirk M. Petruccelli] per-gold=[-4.10]
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.5423 l=0.3248 f=0.5529 N=1.0 g=★ lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
```
(送评归属文本: 14 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.162 l=0.1593 f=0.3768 N=1.0 g=- lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Tupac Shakur〕起始 l_alone(per-gold)=[-3.38]
  块#0: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Kirk M. Petruccelli〕起始 l_alone(per-gold)=[-4.10]
  块#1: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
  双通道小结: 结构必要块=['0', '1'] | 概率移除块=无 | 两通道无冲突
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-4.29]
pF(全轨迹, per-gold)=[-1.67]
通路独立 l [Tempus Unbound] per-gold=[-1.36]
通路独立 l [Missouri River] per-gold=[-1.39]
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.3353 l=0.1855 f=0.3634 N=1.0 g=★ lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.03543 l=0.07754 f=0.2287 N=1.0 g=- lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Tempus Unbound〕起始 l_alone(per-gold)=[-1.36]
  块#0: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Missouri River〕起始 l_alone(per-gold)=[-1.39]
  块#1: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
  双通道小结: 结构必要块=['0', '1'] | 概率移除块=无 | 两通道无冲突
...
打分口径 v2: per-gold 序列 log-likelihood(token SUM, log 域); 判定 ∃gold: d_g≥0.0513(=5%下降) → Informative; d_g≤-0.0488 → harmful 档
p0(无证据, per-gold)=[-8.78]
pF(全轨迹, per-gold)=[-6.04]
通路独立 l [Falkland Islands] per-gold=[-7.02]
通路独立 l [Eastern Time Zone] per-gold=[-5.76]
通路独立 l [United Kingdom, with Dep] per-gold=[-4.76]
...
```
(送评归属文本: 15 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=0.008543 l=-0.01645 f=0.008567 N=1.0 g=★ lineage=✓ necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
```
(送评归属文本: 14 行 = 本块独有新边; 其余行为上一轮已走路径的重渲染,已按三元组归属移除)
◆ 旧标记: cls=effective d=-0.003714 l=0.0002311 f=0.007738 N=1.0 g=- lineage=- necessary=yes
◆ RSCC: 结构必要 → 结构优先保留(不进概率删除)
...
◆ RSCC[v2.1] per-gold 明细:
    Americas                       logP(ref)=-4.762 → logP(cf)=-8.776  Δ=+4.014  drop=+98.2% ★
  max drop=98.19%  coverage=1 D_pos=4.014  label=informative → 保留
◆ LOO:  d(per-gold)=[-1.21]  D_max=-1.215 cov=0
  [结构: necessary=no pathway=falkland islands]

── RSCC 通路隔离倒序收缩迹（每通路独立; d 为 per-gold nats）──
〔通路 Falkland Islands〕起始 l_alone(per-gold)=[-7.02]
  块#0: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 Eastern Time Zone〕起始 l_alone(per-gold)=[-5.76]
  块#1: 结构必要(删除断链) → 结构优先保留 (结构必要=是(删除断链))
〔通路 United Kingdom, with Dep〕起始 l_alone(per-gold)=[-4.76]
  块#2 结构必要=否: d=[+4.01] max-drop=98.19% cov=1 [informative] ⇒ Informative → 保留
  双通道小结: 结构必要块=['0', '1'] | 概率移除块=无 | 两通道无冲突
