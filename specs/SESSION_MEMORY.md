...
- **user 裁定①**:推理流程含后续信息,只给证据会泄露/失真——打分 prompt
  ...
  relations/sg)+归属结果**按执行序编排**,反事实=整段跳过,尾部"基于
  以上信息作答"。
- **user 裁定②**:指标用 token 平均(几何均值,长度去偏);跨轨迹评估用
  **lift=pF−p0**(同 gold 同题,p0 吸收 token 偏置)。修:p0 原为旧口径
  残留,现同口径补算。
- **标本 lift**:2316 0.203 / 2784 0.508 / 21 0.471 / 626 0.176 /
  567 0.539。
- **注意**:P2 四臂全量数字是 evidence-only+桥接口径下测的;终稿仪器
  (编排+p0)如需归档级数字需重跑一轮。
...
- **判决(specs/rscc_p2_report_2026-09-22.md)**:144 轨迹 317 块。
  ...
  (零 LLM 成本);无结构保护时 Static LOO 会遮蔽其中 134(64%)。
  ②终稿规则下顺序臂收敛:LOO↔RSCC 翻转 0/1,FWD↔RSCC 判决 18:1
  ——P1"倒序解除遮蔽"旗舰(2784)是纯概率通道现象,结构优先落地后
  承答保护被结构通道接管。③倒序动态的独立价值=收缩清理
  (30/66 轨迹 pG*>pF)+清理量显著高于随机序(90 vs 35)。
- **claim 修正**:masking 真实存在,但解药主体是结构通道(user 的
  删除断链判定+结构优先裁定);倒序动态是概率通道内的清理机制。
- 待续:对齐机制(P2 数据已备);训练端 P3 暂缓。
...
- **实现**:收割源=ctx.accumulated_triples(交付三元组,含 CVT 穿透边)
  端点 + 每个交付 CVT 节点的 case 图具名邻居(括号属性值的结构形态,
  _full_adj 展开的首个具名邻域)。文本版收割已删。1379 Priest 仍通过,
  153 绿。
...
- **结果**:mean f1 0.6686 / hit 75.0% / NONE 15 / turns 8.0 / failed 0
  ...
  evidence_repeat×5 轨迹、rr 预算×2、乒乓护栏×0(该轮无乒乓样本)。
- **目标家族大胜**(弃权顽固标本对五裁定响应):**1797 0→3/3**,241-s1/
  2576-s1/2209-s1/2784-s2/537-s0/3744-s2 全部 0→1.0。
- **两新伤亡**:①**1379-s0 模型答对 'Priest' 被 off-pool 拒**——渲染有
  profession→Priest 边但合法池(all_candidates/walk_seen)没收录该值,
  弹回后被迫改答池内 'Composer'。根=池完备性缺口(具名属性端点不入池)
  ×弹回机制交互;修法:池并入渲染块的具名端点(行级端点解析)。
  ②2570 3/3→0(答 Truman gold=FDR,判别采样,无机制触发痕迹)。
- 采样方差仍主导(±8 例 1.0↔0.0 双向摆动);-4.9pp 缺口分解:池完备性
  (1379 型)、粒度形(2576 大洲)、判别采样(2570 型)、zh 泄漏损失(~6 轨迹)。

### 2026-09-22 五裁定落地(9bef817)+乒乓护栏(06e7f15):A1短规则口径/A2结构优先/B1三修/B3 worked example
- **A1**:打分口径=短作答规则前缀(5 行,源自 answer_rules_audit)。
- **A2**:结构必要=yes 的块**结构优先保留**(连通性保推理链,概率通道
  不得删);微概率块按绝对判据直移除(保留=有效,移除=有害),护栏改
  **绝对损失**(pF−pG*>τ 才回退,微概率噪声不再回退);空交付块=行为
  无价值直接移除(不再保留为证伪类)。
- **B1**:_sg_served 键序修复(check 取名/mark 取索引,str≠int 永不命中
  ——缓存自 9150a85 起结构性死亡);重复回复改轻量 submission-first
  注记**不回缓存**(字节级重调无信息,评分时移除);rr 每中心预算 3
  (耗尽消息带下一步指令)。重放 reject≠+9=预算门按设计开火。
- **B2.2**:核查确认 rr 错误/NO_EVIDENCE 已含正确动作指令,无需新增。
- **B3**:§28b 加 CASE B worked example(法国邻国标本:台账 9 绑定+
  判别子空→UNKNOWN≠FAIL→交最优支持子集,非 NONE 非并集)。
- **新发现:乒乓绕梯**——模型持 NONE 信念时不调 answer 工具,只反复
  ANSWER_ANALYSIS↔ANSWER_READY 烧满轮数(2576-s0 六弹);梯子只拦
  answer 调用管不到分析循环。护栏:连续第 3 次分析无 answer 调用→
  注入填空模板硬指令(NONE 也显式允许→梯子接管)。实测 17 轮→10-12
  轮,提交行为恢复。塌缩家族冒烟 1/9(残余=2576 大洲粒度形噪声)。
- 153 绿×2;48×3 终跑进行中(reports/v23_final_48x3.json)。

### 2026-09-22 step 覆盖排序(bde6591,user 规格修正):命中数=步数非实体数
- **user 规格澄清**:命中数不是实体命中数,是**模式路径对累积 step 关系集
  的覆盖数**——命中 step1 又 step2 优先于只命中 step2(第二层子图无直连,
  多条模式是常态,穿链覆盖是信号);同覆盖内语义排序;与实体无关。
- **落地**:_derive_multistep_seq 接收锚树累积层集,返回覆盖表;treq 带
  multistep_cov;B 相排序键=(-覆盖, GTE 语义, 长度, 名);每终端 quota 不变。
  声明/层链模式按构造覆盖全 step(每层取一关系)→内部纯语义排序,即设计
  两分句的自然对应。576 探针:step1 调用覆盖恒 1(单层,符合预期);续走
  走链约束路径不经自由派生。153 绿;自洽重放 143/144 不变。

### 2026-09-22 模式家族回归设计(fa30c8d,user 设计重对齐):提交关系名解析,桥永不进终端
- **user 质询"为什么要 direct 桶"**:原始设计=单跳路径直接感知+两三跳
  枚举按(长度,语义)排序→只可能几个模式路径;direct/bridge 桶是名称
  解析/可达性机制(走层),不该进模式层。
- **根因实证**:关系扩展的 direct 桶把桥关系吸收成"直配"(DBG:
  _matched=7 全池)——`film.director.film|film.film.directed_by` 两提交
  →7 关系池→派生终端含 award 族→60 条枚举撞顶→20+ 进 confirmed 渲染。
- **修复**:家族解析三级优先——全名精确匹配→家族/裸名走扩展 direct
  成员(桶的本职)→短名等价;桥永不作终端。实测 derive fam(2)=提交
  对;confirmed 20+→12(残余=SEQ_BRIDGE_TERMINAL=1 站立配置下平凡
  lane 的桥单跳 section;完全去桥历史 A/B -4.4pp,维持现状待议)。
- 153 绿;自洽重放 143/144 不变。

### 2026-09-22 去重对象裁定(60614db):展示变量(三元组)级归属,非文本级
- **user 裁定**:去重不能基于模型侧文本,要对**展示变量(三元组)**去重;
  块的本征内容=本轮游走的**新增三元组**(续走渲染重走根链 step1+step2,
  本质是 step2 的独特新增;上一轮已走的路径信息移除)。
- **实现**:行归属由该行**解析出的边**判定——行内边全为已交付边=纯重
  渲染(丢弃);含≥1 新边的行原样保留(CVT 括号载荷随行保真),其边入
  已交付集。**截断悖论天然解决**:截断版行里没有的边不在其解析三元组
  集,后续完整版带到即交付 NEW(无需归属转移启发式)。
- **标本验证**:567_df97-s0 块#1 现在拥有含 gold 边(Village of the
  Giants)的完整 filmography 行(ratio 0.037 承答保留),块#0=奖项噪声
  (移除后 +50%);21 号案块#0/#1 归属不变正确。
- 文本级两版(行匹配/(rel,head) 键+转移)已废弃——文本皮肤(截断切片/
  内联格式)不可靠。
...
- **①IG 标记词汇**:病根=结构域与概率域被级联焊死、"effective"一词三义;
  ...
  unreached 三条件合取;f 臂/redundant_irr 废弃;substitute 改名 reached。
- **②冗余判定**:33 嫌疑块人审=误标 13(39%)/真冗余 16/存疑 4;**核心
  新发现 H2:N 与价值双向解耦(N=.12 承答块与 N=1.0 真冗余并存),"真
  边界=N+表面变体"不成立**;H7 合取题"删了更好"悖论是 RSCC 修不了的
  核心(626 交集臂);H4 空交付≠空证据两层都接不住;H9 标记跨采样不可
  复现=指标与实际差距的体感来源;D1-D8 人审判据成稿。
- **③解耦验证**:零泄漏实锤;p_yhat 独立责任通道;四象限 RR110/RL32
  (22.2% 检索赢答案输,责任归答案层)/LR0/LL2。
- **④重复头铁**:精确重复 104 次(9.1%)+表面变体 121;**两个真 bug:
  _sg_served 键序错误结构性死亡(check 取名/mark 取索引,一行修)、
  tool:checkpoint 非法工具名致 80 次/41 轨迹误导性拒绝**;间隔 A-B-A
  74 次相邻 gate 拦不到;rr 零防护(预算门只计 sg);五认知成因+提示语
  草案(同命令同结果变体版/两种空的认知学/自检问句)。
- **待裁定**:bug 级三修(键序/checkpoint 反馈/rr 幂等+预算);提示语
  草案 1/2/3/5;RSCC 增补(证伪通道判据改交付族口径/D7 合取保护/V1
  分账/V3 护栏);标记改名;§28b worked example;48×3 重跑。
...
- **审计③结论**:三个 sanctioned 机制(<5 全量/GTE 排序/同属性压缩)全部
  ...
  确定性机制决定:①行层无 CVT→值边通道(626 日期走进了没渲染);
  ②section_bare 裸名排除误杀(537 的 film: 答案面被 actor.film 的裸名
  "film" 排除,GTE 无参与);③_rebuild_paths 单亲 DAG 早发现节点不能作
  终点(**576/1812 的 18/23 名单确定性丢 gold——无采样,纯重建规则**);
  ④行层块预算排序(241 的 Monaco 边在 walk 里但度1环境实体字母序挤占
  +CVT 头边无通道)。整体:渲染层是逐案可复现盲区所在地。
- **修复(22dfeaf,全部值形状判定,不用 answer_type)**:判别值通道
  (CVT→日期/数值行,属主块内+无属主尾部);CVT partner 进块排序+
  上下文尾收 CVT 头边(命名端点为真实走路实体);section_bare 类型化
  (仅当本跳实际渲染 CVT→命名边才排除);终点跳重发现(早发现节点可入
  末层,回溯走原父路径)。验证:153 绿;重放 answer≠3;标本全部复活
  (date_adopted 85 处/[film: 157 处/Monaco|Panama|Barbados 入渲染)。
- **冒烟 2 案×3(终配置)**:2784 **3/3 f1=1.0 Poetic Justice**(此前
  NONE 族+日期不容差双标本,全修);576 s0=**Panama 1.0**(终点重发现
  修复的直接收益),s1 NONE/s2 错选——残余=**检索缺口**(calling_code
  关系族从不进候选列表,审计 P6 已知:curated 列表缺稀有判别族,
  ladder 一轮重选发现不了)。NONE 率 1/6(坏 run 为 19%)。
- **下一个工作流(未动)**:关系族发现缺口(冷门判别关系进不了候选)
  ——retrieval 侧独立工作流;48×3 终判待放行。

### 2026-09-20 V2.3 错配深度审计(3代理,62错案+144v144结构):总报告 specs/mismatch_audit_v23_2026-09-20.md
- **六大错配**:①梯子语义泄漏双路径(ladder 被读作零支持判决推翻模型
  正确的 §28b 应用;二拒绑定清单被逐字当系统指定答案×6,且清单陈旧/有时
  错——372 声称无绑定而 ledger 有);②§28b 两向误用(弃权侧 ANY-binding
  范围扩展×10"technically we have bindings…still NONE";提交侧 union
  dump×7 全交毁精度);③渲染欠 spec(判别值在 CVT 内未渲染×6+、同关系
  采样间尾集方差 gold 被滤、slot 不按 answer_type 选键);④链式单树
  (sg/plan 1.92→1.40,断链即全断→44 例进梯子;MULTI-TREE 提醒反而 80%
  hit);⑤门组合病(per-sg 预算 0→15 拒;answer-var 语法弹回年份值后
  rescue 又捞回被拒值——硬不变式被击穿;workflow-order 连拒×3;
  checkpoint 语法×4 烧 6 轮);⑥zh 移除隐性代价(5/19 题读法退化——
  zh 同时是语义压缩器)。
- **结构判决**:+2 轮中 75% 在首答之后(答案弹回率 57.5% vs 11.3%);
  27 NONE 中 25 走梯子 0 转化;26/27 死轮上限;21/27 gold 就在输出里。
- **修复清单(杠杆序)**:梯子中立化/二拒最新层重算或绑定直接终局;
  §28b 操作化双向改写+容差+名内取值;rescue 不捞被拒值+预算 per-fact;
  多树回 plan 时规则;渲染三修;门放宽两处;zh 角色重审(待裁定)。
  正面保留:MULTI-TREE(80%)/NO_EVIDENCE(56%)/打包/语义槽修复。
...
- **判决定案**: cmd_rewalk 48×3 = **f1 0.6576 / hit 73.6%**(基准 0.7177/
  ...
  f1=1.0 不可信(冒烟只验跑通)。OUT=reports/cmd_rewalk_48x3.json。
- **根因(量化)**:判别密度崩塌——hops/轨迹 32.0→24.7,块 15.9→13.1;
  **掉 hops>10 的 44 条均值 f1 −0.108**(占回归大头 ~4.8pp),hops 持平
  组也 −0.049(排序内容本身小代价)。机制链:完整性排序+每终点 top-3+
  步序丢弃 → 模式更少更长 → 判别上下文变薄 → 相近候选误提交
  (1840: North vs South Saami)。**与 2026-09-12 SEQ_PAT_SEMANTIC A/B
  历史教训同构:高扇出模式带着超 gold 的判别上下文,选择激进必亏**。
- **已回退**(用户预定授权"不行就会退"): seq_tools/test_rel_seq 行为码
  checkout 回 790f6d5(链 90a219a→7ab5ac2 的 kgqa/tests 部分撤销);
  **保留**: 重放提速(replay_dispatch GTE 窗口,30s)、全部文档/裁定/
  render_diff/轨迹文档、判别器结论。验证: 153 绿;144 重放 30s;
  answer≠3/144。
- **重试方向(待用户裁定,需再一轮 LLM 48×3)**: 完整性只作**排序键**
  不作**收口器**——恢复基准证据密度(每终点配额对部分链照常发满、
  步序违规只降序不硬弃、组合链不占额外配额),完整组合链置顶但不挤占;
  判别密度是第一约束。机制语义(步表/骨架/命令修正)文档全在,重构
  成本低。
- 教训:**冒烟 2case 全对 ≠ 可上线;行为改动必须 48×3 终判**(本页首例)。
...
- **设计锚点(用户 2026-09-20 三)**:原始设计本来就简单——核心关系
  →三元组,CVT 部分即潜在候选,**模型从渲染面挑起点本身就是探索延长
  机制**。完成集是树续接迭代出来的隐藏第二真相源(层走),判定读它而
  不读模型实际看到/挑选的面——两源语义漂移(并联一跳 vs 链叶)即耦合
  问题全部根源。**统一:判定逻辑读链叶集(=confirmed 链终点∪CVT 属性
  叶,与候选/渲染同源同份)**;center∈链叶⇒EXTEND 从产出该叶的具体链
  延长,center==根⇒UPDATE;层走完成集对判定退役(五消费者:匹配/
  classify 可行性/UPDATE-EXTEND/direct 步起点/echo 全部改读链叶集)。
  叶集被**选中链**天然限界——非闭包,层内接力爆炸(empty 5→187)不会
  复现,渲染准入预算=选择层控制原样适用。
...
### 2026-09-16 inline-CVT 渲染全量判决
- f1=0.6539/hit=75.0% — 仍低于无实体块的 fullstack(0.6967)
- **但 027d777f runtime→1.00、deflator→0.89** — 判别器家族最强轮
- 逐 case 对比: 027d777f 1.00 + d25753d2 0.89 + d9206043 0.83
  vs 24353bbc 0.68 + 1278d3da 0.32 + c7fffe30 0.33
- **结论: 实体块渲染(无论 v1 还是 inline)总分都低于模式段平铺**:
  fullstack(0.6967) > entity-v1(0.6606) > inline(0.6539)
  但判别器单 case 最高分都在实体块版本
- **混合渲染方向最有潜力**: 模式段做主体 + 实体块内 CVT 用 inline
  格式 — 但这是下一迭代的活
- 当前最优站立 = fullstack(无实体块) 0.6967/79.2%

### 2026-09-16 混合渲染判决(实体块主体+inline CVT+去重, 671fe6d)
- **f1=0.6849/hit=79.2%** — 实体块系列最高(v1 0.6606→inline-pattern
  0.6539→hybrid 0.6849), 距模式段站立(0.6967)仅 -1.2pp
- hit 79.2% **与模式段持平**
- 判别器: deflator **1.00** + 24353bbc **0.87** + 1278d3da **0.67**
  (三个标本同时达到历史高位的首次)
- 渲染版本对比:
  | 版本 | f1 | hit |
  |---|---|---|
  | 模式段(fullstack) | 0.6967 | 79.2 |
  | 实体块v1 | 0.6606 | 75.7 |
  | inline(pattern) | 0.6539 | 75.0 |
  | **hybrid(本轮)** | **0.6849** | **79.2** |
- 实体块方向逐步收敛; 保留为候选站立(差模式段1.2pp但判别器更强)

### 2026-09-16 答案层 V6 COUNT CONTRACT 落地(a4db54d)
- **用户设计**:"先找每个子问题的答案看符合要求的有那些,没有完美
  符合的就答出当前规则中最好的"
- §10 ANSWER_ANALYSIS 加 COUNT_CONTRACT 行(CASE A=交全部全满足 /
  CASE B=交最强单个);§11 重写——砍 "incomparable→keep both"
  (平局过溢根因),判别器必须算值取极值
- **迷你轮: 1278d3da 0.67→0.89**(s0/s2=1.00 答 Belgium,
  不再交 9 国全家桶); 24353bbc 0.75
- 全量 48×3 运行中
- **V6 全量 48×3**: f1=0.6842/hit=78.5% — 带内(与 hybrid 0.6849 持平),
  未超模式段站立(0.6967)
- **1278d3da 0.78**(迷你轮 0.89)— V6 契约生效但全量有 churn
- **ab5818ba 0→0.67**(Jesus Christ 题历史突破)
- **d9206043 0.50→0.17**(Saami 回落,churn)
- 判别器家族总增幅 vs 基线: d25753d2+0.55 / ab5818ba+0.67 /
  1278d3da+0.02 / 6d6f2ca8+0.33 — 方向正确
- **V6 + 实体块的组合还需微调**(24353bbc 0.79→0.57 可能是
  V6 的 CASE B 单选在多 gold 题上过度收缩)

### 2026-09-16 环境纪律适配教训：一揽子移除护栏=灾难
- **一揽子回退后 mini 恢复**: 1278d3da 0.33(s1=1.0)/24353bbc 0.77
- **教训**: 199 次"拦截"中大量是**意外有用的护栏**——
  - literal-nudge(92x): 强迫模型用 ?var=携带全部候选 → 移除后模型
    只走单个字面实体的边,判别信息不全
  - unbound-var error(21x): 阻止未声明调用 → 移除后自动展开到
    错误的完成集(含噪声)
  - purity-loop(16x): 3 次相似查询警告 → 放宽到 5 次后模型烧更多
    预算在相似查询上
- **正确方向**: 不是移除护栏,而是**精细化**——literal-nudge 只在
  实体不在树内时触发;unbound-var 只在有正确树时自动展开;
  purity 阈值不变但在关系不同时不计数
- 当前状态 = 回退到 V6+hybrid(0.6842/78.5%)

### 2026-09-16 环境提示对齐终版：f1 0.7177/hit 81.9% 历史新高
- **全部版本对比**:
  | 版本 | f1 | hit |
  |---|---|---|
  | fullstack (pattern) | 0.6967 | 79.2 |
  | hybrid entity-block | 0.6849 | 79.2 |
  | hybrid + V6 | 0.6842 | 78.5 |
  | hybrid + V6 + 移除护栏 | 0.4757 | 58.3 ← 灾难 |
  | **hybrid + V6 + 对齐提示** | **0.7177** | **81.9** ← 新高 |
  | 单步栈 | 0.6826 | 80.6 |
- **判别器家族**: deflator 1.00 + Saami **1.00** + tvrage 0.67 +
  **runtime 1.00**(首次满分)——四家同时高位
...
### 2026-09-17 标注体系元评估(18c3f6f):一处不合理已修,三点待裁定
- **已修(内部矛盾)**:8 个 (irrelevant, necessary=yes) 操作重标
  helpful-midchain——中间链桥价值(交付 gold 边所挂 hop 实体,7/8 在
  f1=1.0 轨迹)。修正规则:**必要性高于 Tier-1 档位**(零覆盖零引用
  但删除断连 ⇒ 有效)。交叉矛盾清零。
- **待裁定 1 hub-gold 40%**:57/144 轨迹 gold 图度>100,φ/断链不可靠。
- **待裁定 2 预测力弱**:轨迹级 helpful-share 与 f1 非单调;结局解释
...
- **◆ 行精简为四核心值**(1a0c63d,用户裁定 09-18):结构必要 +
  case 头 p0/pF + 每块 [p_alone=独立概率 p_removed=移除后整体概率
  档]——d/l/f/N/g/台阶退到 JSON 不再上 ◆ 行。**精度坑**:打分侧
  round(x,4) 把 e-05 级概率抹成 0(d 全零的假象),改 4 位有效数字
  float(f"{x:.4g}"),dump 侧 <1e-3 用科学计数。1379 重打分后各块
  p_alone 3.6e-06~8.8e-06 可见。分档分布不变(33/5/3)。
- 13 轨迹全量: 每条 p0/pF/Δ + 每行为 N/f/d/l/g/lin/p⁻/p_alone + 调用
  ...
  ——irr 门槛需加"表面变体"条件, 仅 N 高 f 低不构成 irr, 新增
  exploration-harmless 档(待裁定)。
- 两个结构性发现: ①**空交付块异常**(1731 mod0: N=0.00 交付空
  l=+0.097 全场最高——空结果携带"此路不通"信息, 打分语义需单独
  定义); ②**替代对 LOO 互顶签名**(626 mod1/mod2 d/l 双大负但
  effective 正确=§10.8; 与 567 mod5 真验证步 l=−0.088 需区分)。
- 最强对照: 2784 s0 vs s2 同证据(pF .51/.55)采样决定答案对错——
  CASE-B 抛硬币; 567_df97 检索 54.7x 答错=参数覆盖; 1797-s1 检索
  负贡献(pF<p0, mod0 harmful 三条件被 L1 短路救回)。
- **检索概率优势台账**(概率与答案脱钩):567_df97(pF=.629/f1=0)与
  2784-s2(.548/0)=检索赢答案输;检索很少是瓶颈(95% gold 文本可见),
  损失集中答案层(拒答死锁/参数覆盖/UNRESOLVED≠CONTRADICTED)。
- **冗余判定核心漏洞**:redundant_irr 系统性误标有价值探索(人机
  分歧 4/4,567_11fd-mod1 N=.90 也被标 irr)——**f 类标准是伪装的
  答案相关性**;dup(N≈0)可靠。真边界=N+表面变体,不是 f。另 7 洞:
  含金噪声不可记录(567-[5] l<0 仍 effective)/L1 短路救 harmful/
  g 首现vs首取冲突/替代对账未平/per-pathway 双计/基准震荡不可见/
  hub-gold 旧账。
- **重复/头铁**:69/1088 浪费(6.3%),集中 4 条烧满预算轨迹。
  检索侧干净(被拒后 0 原样重试);**最贵模式=答案侧头铁——对着
  Second refusal 打印的 bindings 连续 answer NONE 8-9 次到预算死**。
  _sg_served 语义幂等 144 轨迹**零命中(死机制)**;21 组表面变体
  (短/全关系名、名字/m.id、typed/?var)绕过精确 gate。
- **修复杠杆排序**:refusal 三级升级+echo 服从教学 > 解析键 gate+
  表面等价教学 > irr 判定改 N 边界 > restart 证据保留。
  5 条提示语(§7 停教改写/§11 echo 服从/§0 表面等价/repeat 行动化/
  同证据自检)+3 条 harness 快赢已列审计文档。
...
- 探针初验:部分采样双关系提交(s0 f1=1.0),多数仍单选——概率性遵守,
  48×3 待跑。
- **31 未满分 case 重跑**(v24e,reports/v24e_fixcheck_31.json):子集均分
  0.427→**0.537(+11pp)**,gain14/loss7。**576 0.00→1.00**(多关系提交带
  来对照证据,大段丢失型完全恢复);2784/372/1840 →1.00;回退:25_db96
  (-0.67)/241_97bf(-0.56)/567_df97(采样波动或多选噪声,待查)。
  估算全量≈0.66-0.67≈基线恢复。
- **join 回归+id 透明测试**(v24f,31 案):0.427→0.498(gain12/loss6,方差带
  内);241_97bf→1.00 恢复;576 本轮 0.33(v24e 曾 1.00,采样波动);25_db96
...
- **v25 全量(所有修复)**:0.601/69.4%(基线 0.669/75.0,gain11/loss20)——**净负向**。
  单案受益:21_ 1.00/2152 1.00/25_db96 0.67/626/537 1.00;掉分:1171 0.33
  (v24c 曾 1.00)/567_df97 0/576 0.67/1731 0.50。**merge bounce 副作用实证**:
  触发 6/144,其中 5 次后答错(1171/567 s0 均被 bounce 后答 NONE/单事实)——
...
- **v27(树键归一+中段 CVT 链 >= 修复,eb604a5)**:0.626/71.5%(v26 0.517→
  +10.9pp;基线 0.669 差 -4.3pp;vs v24c 0.647 差 -2.1pp,gain14/loss12)。
...
- **v28(CVT 内联+终跳边修复,caea2b4)**:0.645/72.9%(基线 0.669 差 -2.4pp;
...
- **重设计 A+B 落地+v31(47594c7)**:①render=selected(收集层 ADMIT cap 删,
  显示集=选中集,65 模式外/93 丢失边双向闭合);②全局 GTE top-5 总排序选择
...
- **修法建议(仅报告,未动管线)**,按杠杆排序:①`_rebuild_paths` 末跳
  ...
  topk=6 即入)。
- **2540 归因(子代理 B,db9ddcd)+修法① 落地(f1a8609)+v33**:三层复合损失
  (选择层被挤差2位非主因/walk 层末跳入边不 materialize 最关键/续走层
  influenced_by 结构死路);R2 判决=v23 取胜序列在 v31 机制下 gold 0/2
  (机制侧回归,窗口 v25,主嫌 lane-2 删除)。侧发现:cov 键型死键(读写
  键不匹配恒 0,step 覆盖从未生效);kwconnection hub 家族携带 gold 反向
  边被当噪声。修法①(入边 materialize,镜像连续性)落地:2540 0→0.22
...
- **核心机制对齐修正(2cabd8a,user charter 重申)+v34**:④'cov 键型修复
  (写 idx 元组/读名字串→经 cmap 的 pk 解析,step 覆盖主键首次生效;
  tiebreak 撤回——违反从头到尾逐步检测的核心);③'delta 改精确三元组
  (方向归一保留,逆关系折叠回滚——"完全对应");②'撤回(方向严格对应,
  gold 可达性靠方向正确模式在选择层胜出,由④'服务)。
  **v34=0.703/78.5%——hit 新高**(v31 77.8→78.5;153 绿+重放器完好)。
- **IG 评估双修正(2026-09-26,user 审计驱动)+v34ig2 重跑**:①跨通路去重
  bug——_delivered 原为全局单集,sg1 的边把 sg2 的同行从评分文本剥掉,
  反事实(sg1 移除)时这些边从整个 prompt 消失,sg2 被不完整文本不公平
  评估;改为 per-pathway 所有权(同行只在同通路早块已交付时剥)。②单块
  通路结构判死——仅 1 个可评块的通路若 necessary=no/n/a 直接移除,不
  进概率(user:结构连通性是必要条件)。v34ig2 分布:结构 179/单块判死
  116/informative 13/redundant 5/harmful 4/abstain 7——概率通道裁决
  范围大幅收缩(从 73+62+6 到 13+5+4+7)。
- **轮数审计(v34)**:正确案 mean=7.6 turns vs 错误案 mean=8.9(p90=12,
  max=17)——错误案轮数确实偏高但差 1.3 轮;精神洁癖(必须凑齐全部候选
  才推进)是认知侧待优化(576 七国/Ron Howard 全导列表标本)。
- **live 渲染跨通路压制:零实例**(v34 同 fid 后空渲染=0,近空=1 且 f1=1)
  ——用户审的 sg2 空是 RSCC 评分文本侧非 live 渲染。
